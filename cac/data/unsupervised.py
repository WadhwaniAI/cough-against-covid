"""
Defines UnsupervisedDataset class which is used for unsupervised
learning tasks where each input has a multiple implicit labels (attributes).
"""
...
class UnsupervisedDataset(BaseDataset):
    """Dataset class for unsupervised learning
    ...
    def __init__(self, dataset_config: List[DatasetConfigDict],
                 target_transform: Any = None, signal_transform: DataProcessor = None,
                 fraction: float = 1.0, as_frames: bool = False,
                 frame_length: int = None,
                 hop_length: int = 500, min_length: int = 100):
    ...
    def _check_item(self, index: int, item: AudioItem):
        assert 'unsupervised' in item.label,\
            "Item at index {} does not contain 'unsupervised' in label".format(index)

    def __len__(self):
        return len(self.items)


class UnsupervisedDatasetBuilder:
    """Builds a UnsupervisedDataset object"""
