"""Script for inference for a given config/user/epoch that follows our multi-signal training on given dataset.

Example:
$ cfg=defaults/multi-signal-training.yml
$ For Cough and Context Model
$ python inference_multi_signal.py -v $cfg -e 0 -u piyush --data_versions v9.8 -m test --at softmax
$ For Cough and Voice Model
$ cfg=experiments/multi-signal-training/cough-voice/naive/multi-signal-min.yml
$ python inference_multi_signal.py -v $cfg -e 0 -u piyush --data_versions v9.8 v9.8_voice_repeated -m test --at softmax
...
class InferenceOnMultiSignalDataset:
    """Class for inference of a model on a given multi-signal dataset."""
    def __init__(self, args):
        super(InferenceOnMultiSignalDataset, self).__init__()
        # load config
    ...
    def load_and_set_config(self, args):
        ...
        assert len(config.data['signal_wise_data_cfgs']) == len(args.data_versions),\
         "Provide different versions per signal"

        # add `test` field copying from `val`
        signal_wise_data_cfgs = config.data['signal_wise_data_cfgs']
        for i in range(len(config.data['signal_wise_data_cfgs'])):
            config.data['signal_wise_data_cfgs'][i] = self.add_test_mode_to_config(
                        config.data['signal_wise_data_cfgs'][i], [['dataset', 'params'], ['signal_transform']])
        ...
        for i in range(len(config.data['signal_wise_data_cfgs'])):
            config.data['signal_wise_data_cfgs'][i]['dataset']['config'] = [
                {
                    "name": args.data_name,
                    "version": args.data_versions[i],
                    "mode": args.mode
                }
            ]
    ...
    def load_data(self, config, args):
        dataloader, _ = get_dataloader_multi_signal(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_versions', nargs="+", default=["v9.8"],
                        help='specifies the version of data to evaluate on for each signal')                    
    ...
    inference = InferenceOnMultiSignalDataset(args)
