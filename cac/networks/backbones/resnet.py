...
from cac.networks.activations import factory as activation_factory
...
def _resnet(arch, block, layers, in_channels, pretrained, progress, **kwargs):
    ...
    if pretrained:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        # modify loaded state_dict to only retain those weights
        # where the shapes match with model's state_dict
...
def resnext50_32x4d(in_channels=3, pretrained=False, progress=True, **kwargs):
    ...
    return _resnet('resnext50_32x4d', Bottleneck, [3, 4, 6, 3],
                   in_channels, pretrained, progress, **kwargs)
